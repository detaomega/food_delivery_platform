<?php
    function distance($lat1, $lon1, $lat2, $lon2, $unit) {
        if ($lat1 == $lat2 && $lon1 == $lon2) return 0.0001;
        $theta = $lon1 - $lon2;
    ...
    try {
        ...
        if ($priceLow != "" || $priceHigh != "" || $mealSearch != "") {
            $stmt=$conn->prepare("select store.name, category, position_longitude, position_latitude, store.ID from store, product " . $condp . " group by store.name");
            ...
            $stmt=$conn->prepare("select * from store " . $cond);
            $stmt->execute($condArray);
            $res = ["error" => false];
            $i = 0;
            while ($row = $stmt->fetch()) {
